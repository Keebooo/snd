...
function App() {
  ...
  const getRoleForDate = (date: Date): RoleData => {
    // Reference date: 31 Agustus 2025 = Zippy (index 0)
    // Menggunakan UTC untuk konsistensi
    const referenceDate = new Date(Date.UTC(2025, 7, 31)); // Month is 0-indexed, so 7 = August
    
    // Convert input date to WIB timezone
    const wibOffset = 7 * 60; // WIB is UTC+7
    const utc = date.getTime() + (date.getTimezoneOffset() * 60000);
    const wibTime = new Date(utc + (wibOffset * 60000));
    
    // Get date at midnight WIB (normalize to start of day)
    const targetDate = new Date(Date.UTC(wibTime.getFullYear(), wibTime.getMonth(), wibTime.getDate()));
    
    // Calculate difference in days from reference date
    const timeDiff = targetDate.getTime() - referenceDate.getTime();
    const dayDiff = Math.floor(timeDiff / (1000 * 60 * 60 * 24));
    
    // Calculate role index (Zippy starts at index 0 on reference date)
    let roleIndex = dayDiff % roles.length;
    if (roleIndex < 0) {
